...
class FakeAccessPoint(object):
    class FakeBeaconTransmitter(threading.Thread):
        def __init__(self, ap):						#konstruktor
        ...
        def run(self):
            while True:
                for ssid in self.ap.ssids:
                    self.ap.callbacks.cb_dot11_beacon(ssid) #Triggered every 0.1 seconds. The default behaviour is to send a Beacon frame.
    ...
    @classmethod
    def from_file(cls, path):		#konfiguriranje 
        conf = Conf(path)
