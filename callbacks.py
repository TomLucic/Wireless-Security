...
class Callbacks(object):
    def __init__(self, ap):
        self.ap = ap
										# The default behaviour im je svima da odgovore sa tim istim frameom. (ovima ispod)
        self.cb_recv_pkt = self.recv_pkt		#Triggered every time a packet is received. This callback defines when all other callbacks are triggered.
        self.cb_dot11_probe_req = self.dot11_probe_resp	#Triggered on reception of a Probe Request frame. 
        self.cb_dot11_beacon = self.dot11_beacon
        self.cb_dot11_auth = self.dot11_auth	#Triggered on reception of an Authentication Request frame. 
        self.cb_dot11_ack = self.dot11_ack	
        self.cb_dot11_assoc_req = self.dot11_assoc_resp	#Triggered on reception of an Association Request frame. 
        ...
        self.cb_tint_read = self.recv_pkt_tint	#Triggered on reception of a packet from the fakeap virtual interface.

    def recv_pkt(self, packet):
        try:
            if len(packet.notdecoded[8:9]) > 0:  # Driver sent radiotap header flags
                # This means it doesn't drop packets with a bad FCS itself -> fcs je za detektanje erorra
